...
#    `functionName` is a function's name in your module (e.g., `historicalFireAndHarvestEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Git/historicalFireAndHarvest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("historicalFireAndHarvest")
  path <- list(modulePath = "C:/Ian/Git",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    historicalFireAndHarvest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("historicalFireAndHarvestEvent1", envir = .GlobalEnv)) {
    simOutput <- historicalFireAndHarvestEvent1(mySim)
  } else {
  ...
  if (exists("historicalFireAndHarvestEvent2", envir = .GlobalEnv)) {
    simOutput <- historicalFireAndHarvestEvent2(mySim)
  } else {
